...
def process():
    ...
    with tracer.start_as_current_span("service-b.process") as span:
        # RCA normalization
        span.set_attribute("rca.node", True)
        span.set_attribute("rca.service", "service-b")
        span.set_attribute("rca.role", "intermediate")
        ...
        if random.random() < FAIL_RATE:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Service-B simulated failure | {incident_id}"
            )

        # Downstream call
        try:
            response = requests.post(f"{SERVICE_C_URL}/process", timeout=2)
            ...
            raise HTTPException(
                status_code=502,
                detail=f"Service-B failed calling service-c | {incident_id}"
            )
